...
Код с множеством goto, копипастой и плохой структурой. Специальная структура программного обеспечения затрудняет расширение и оптимизацию кода. Частый рефакторинг может улучшить структуру и поддержку.
...
Повторное использование программного кода методом вырезания и вставки, без переиспользования. Приводит к серьёзным проблемам сопровождения. Может быть как экономией времени, так и источником проблем.
...
Код копируется из проекта в проект с минимальными изменениями, с большим количеством закомментированных или устаревших функций. В итоге образуются огромные блоки мёртвого кода, который боятся удалить. Особенно распространён в аутсорсинговых компаниях.
...
Отсутствие чёткого разделения, что должно быть настраиваемым, а что — жёстко заданным в коде.
