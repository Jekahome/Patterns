...
// cargo run --bin fabric_method -- --example ex1
fn main(){
    let mut pizza = new_pizza(&mut (Box::new(NYPizzaConcreteFactory) as Box<PizzaFactory>), PizzaType::CHEESE);
    pizza = new_pizza(&mut (Box::new(NYPizzaConcreteFactory) as Box<PizzaFactory>), PizzaType::PEPPERONI);

    pizza = new_pizza(&mut (Box::new(ChicagoPizzaConcreteFactory) as Box<PizzaFactory>), PizzaType::CHEESE);
    pizza = new_pizza(&mut (Box::new(ChicagoPizzaConcreteFactory) as Box<PizzaFactory>), PizzaType::PEPPERONI);
}
