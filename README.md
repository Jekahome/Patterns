...
* [Code review](https://github.com/Jekahome/Patterns#code-review)
...
Так как код читают в **5–10 раз чаще, чем пишут**, хорошее имя часто позволяет понять, что происходит, **не заглядывая в реализацию**.

Удачно подобранное имя:

* сразу передаёт смысл данных и устраняет неоднозначность
* снижает когнитивную нагрузку, устраняет необходимость знания контекста
* уменьшает количество ошибок
* делает код самодокументируемым
* ускоряет ревью
* упрощает рефакторинг

Плохие имена заставляют:
...
#### Имя должно соответствовать уровню абстракции, на котором оно используется

* если код решает **бизнес-задачу** — имена должны быть бизнесовыми
* если код решает **техническую задачу** — имена должны быть техническими

Если в бизнес-логику проникло имя, отображающее детали реализации, следует обратить внимание на архитектуру.

#### Публичные имена не должны раскрывать детали реализации

Публичное имя — это **часть контракта**, а не реализации. Оно должно говорить, **что делает код**, а не **как это сделано**.

Если публичное имя раскрывает конкретику, вы привязываете пользователя API к текущей реализации.

Примеры **неудачных публичных имён**:
...
Иногда конкретика допустима, если она **составляет смысл API**.
 
#### Будьте последовательны — придерживайтесь единого стиля

Признаки «грязного кода» в именовании:

* стиль меняется от файла к файлу
* зависит от автора
* не подчиняется правилам

Примеры плохой консистентности:
...
Или методы, выполняющие одно и то же но с разным стилем:
...
* `flag` — плохо
* `dataReady` — лучше
* `currentCustomerNumber` — лучше

#### Порядок слов имеет значение

* `NumCustomer` — общее количество клиентов
* `CustomerNum` — номер текущего клиента
  
#### Именование констант

* `FIVE = 5.5` — плохое имя
* `CYCLE_NEEDED` — лучше
* `MAX_RETRY_COUNT` — лучше
...
### фон GTD
[Getting Things Done (GTD)](https://en.wikipedia.org/wiki/Getting_Things_Done) — это система личной эффективности Дэвида Аллена, суть которой — выносить из головы всё, что нужно сделать, во внешнюю систему, чтобы освободить ум для концентрации на самой работе. Это достигается через пять этапов: *сбор → прояснение → организация → обзор → выполнение.*

Метод предназначен для уменьшения стресса и повышения продуктивности, помогает справляться с большим количеством задач и проектов.
...
Хорошее код-ревью анализирует изменения и их соответствие общей структуре кода. Оно проверяет понятность названия и описания, а также причину внесения того или иного изменения. Рецензия включает в себя  ...

[Code_review.md](https://github.com/Jekahome/Patterns/blob/main/Code_review.md)
...
**Недостатки**. Будущие программисты, читающие это в коде, не будут ожидать, что это произойдет.
...
**Обсуждение**. Нет ни одной хорошей альтернативы. В зависимости от конкретных обстоятельств может быть лучше повторно реализовать использование признаков или написать фасадные методы для ручной диспетчирезации Foo. 
...
* Внедрение через конструктор (Constructor Injection) — самый предпочтительный, обязательный и явный способ. Зависимость требуется для существования объекта.
* Внедрение через свойство/сеттер (Setter/Property Injection) — зависимость может быть опциональной или изменяемой после создания объекта.
* Внедрение через метод (Method Injection) — зависимость нужна только для выполнения конкретного метода.

"Локатор служб" (Service Locator) — это альтернативный, но часто считающийся антипаттерном, шаблон для управления зависимостями. Ключевое отличие от DI:
* DI (Внедрение зависимостей): Класс пассивно получает зависимости извне ("Don't call us, we'll call you"). Это делает зависимости явными (особенно в конструкторе).
* Service Locator: Класс активно запрашивает зависимости из глобального или доступного контейнера ("Дайте мне то, что мне нужно"). Это скрывает зависимости, делая код менее прозрачным и усложняя тестирование.

Контейнер внедрения зависимостей (DI Container) — это тот самый "общий механизм", который автоматизирует создание объектов и разрешение их зависимостей. Он знает, как создавать сервисы и что куда передавать (на основе конфигурации, атрибутов или авторегистрации).
...
...читать Eric Evans’s Domain-Driven Design or Martin Fowler’s Patterns of Enterprise Application Architecture

1. **Архитектуры по "форме" приложения**

Это структурные стили — описывают, как приложение разделено на части и как они взаимодействуют.

| Стиль                                           | Суть                                                                                  | Пример                                                  |
| ----------------------------------------------- | ------------------------------------------------------------------------------------- | ------------------------------------------------------- |
| Monolithic                                  | Всё приложение — один большой блок (код, логика, UI, база).                           | Классическое веб-приложение на Django или Laravel.      |
| Layered architecture (многослойная)         | Разделение на слои: UI → Service → Domain → Data.                                     | Типичная корпоративная Java-архитектура.                |
| Hexagonal Architecture (Ports and Adapters) | Бизнес-логика (ядро) отделена от внешних систем (БД, API, UI) через порты и адаптеры. | Чистая архитектура в стиле DDD, популярна в Rust и Go.  |
| MVC (Model–View–Controller)                 | Разделение на модель (данные), представление (UI) и контроллер (логика).              | Классика для UI и веб-фреймворков (Rails, ASP.NET MVC). |

Эти стили можно комбинировать: например, может быть Hexagonal Monolith, который внутри реализует MVC.

2. **Архитектуры по распределению**

Описывают взаимодействие между узлами в распределённой системе.

| Стиль                  | Суть                                                      | Пример                                       |
| ---------------------- | --------------------------------------------------------- | -------------------------------------------- |
| Client–Server      | Клиент обращается к серверу, который отвечает.            | Веб-браузер ↔️ сервер.                       |
| Microservices      | Много независимых сервисов, каждый выполняет свою задачу. | Netflix, Amazon, микросервисы на Kubernetes. |
| Peer-to-Peer (P2P) | Все узлы равноправны, нет центрального сервера.           | BitTorrent, блокчейн.                        |

- Client–Server — базовый шаблон,
- Microservices — его развитие (много серверов),
- P2P — противоположная модель (нет центрального узла).

3. **Архитектуры по взаимодействию (коммуникации)**

Описывают как компоненты обмениваются данными.

| Стиль                                               | Суть                                                                                   | Пример                                     |
| --------------------------------------------------- | -------------------------------------------------------------------------------------- | ------------------------------------------ |
| Event-Driven Architecture (EDA)                 | Компоненты реагируют на события (асинхронно).                                          | Kafka, RabbitMQ.                           |
| Publish–Subscribe (Pub/Sub)                     | Отправитель публикует события, подписчики слушают.                                     | MQTT, Redis Pub/Sub.                       |
| CQRS (Command Query Responsibility Segregation) | Разделение операций чтения и записи данных.                                            | Отдельные модели/БД для команд и запросов. |
| Event Sourcing                                  | Вместо хранения состояния — сохраняются все события, из которых оно восстанавливается. | История заказов или транзакций.            |

- CQRS и Event Sourcing часто идут в паре
- EDA и Pub/Sub — это механизмы коммуникации (Pub/Sub — частный случай EDA)


4. **Архитектуры по проектированию домена**

[DDD.md](/DDD.md)

Они определяют как организовать бизнес-логику.

| Стиль                          | Суть                                                                     | Пример                           |
| ------------------------------ | ------------------------------------------------------------------------ | -------------------------------- |
| DDD (Domain-Driven Design) | Сосредоточено на бизнес-модели и предметной области, а не на технологии. | Используется с Hexagonal и CQRS. |

DDD часто комбинируется с Hexagonal, CQRS, Event Sourcing.

Итого: визуальная группировка

| Категория                     | Примеры                                     |
| ----------------------------- | ------------------------------------------- |
| Структура приложения      | Monolithic, Layered, Hexagonal, MVC         |
| Распределённость          | Client-Server, Microservices, Peer-to-Peer  |
| Коммуникация              | Event-Driven, Pub/Sub, CQRS, Event Sourcing |
| Модель предметной области | DDD                                         |


Пример сочетания в реальной системе заказов:
* Архитектура: **Hexagonal + DDD**
* Взаимодействие между сервисами: **Event-Driven (Kafka)**
* Каждый сервис: **CQRS + Event Sourcing**
* Разделение сервисов: **Microservices**
* UI: **MVC** во фронтенде
* Общая форма деплоя: **Client–Server**
...
Этот слой отвечает за отображение данных пользователю и обработку пользовательского ввода. Включает в себя пользовательский интерфейс, отображение данных и обработку событий. Зависит от слоя бизнес-логики.

##### Бизнес-логика (Business Logic Layer или Service Layer):
Здесь содержится основная бизнес-логика приложения. Обрабатывает запросы от представления и координирует выполнение бизнес-правил и задач. Не зависит напрямую от конкретной реализации базы данных или представления.

##### Слой данных (Data Layer):
Этот слой управляет доступом к данным и их хранением. Включает в себя работу с базой данных, файлами, внешними API и т.д. Зависит от бизнес-логики. Преимущества слоенной архитектуры включают:
