...
* #### Правила комментариев
    ...
    Использование TODO: Маркер кода TODO часто используется для обозначения незавершенных задач в коде, и в его структуру можно добавить информацию об идентификаторе задачи или владельце для более точного отслеживания и управления.
        ```
        // TODO [ISSUES-101] (Owner: Bob) Refactor this function
...
В Rust полиморфизм реализуется несколько иначе, чем в классических ООП-языках (например, Java/C++), но основные виды поддерживаются с помощью **трэйтов, обобщённых типов (generics) и системы владения**.  
...
1. **Поток лавы (Lava Flow)** 

Возникает когда код копируется из проекта в проект с минимальными изменениями - комментированием части функций, небольшими доработками. 
В результате образуются огромные блоки закомментированного кода, функции работающие только с частью параметров, сложные обходы. Особенно распространен в аутсорсинговых компаниях.

2. **Blob (Божественный объект)**

Гигантский класс, который делает все и становится почти операционной системой. Автор отмечает, что встречал такое в основном в банковском легаси. 
За таким "блобом" обычно скрываются десятки других плохих практик.

3. **Непрерывное устаревание**

Использование устаревших технологий без осознанного выбора. Особенно плохо, когда поддержку компонента приходится оказывать самой команде разработки.

4. **Функциональная декомпозиция**

Разделение классов не по функциональному уровню, а по частям бизнес-процесса. 
Приводит к смешиванию уровней - фронт в бэке, запросы к веб-сервисам в функциях сложения чисел и т.д.

5. **Лодочный якорь (Boat Anchor)**

Наследование большого количества бесполезного кода, который еще и мешает развитию. Часто возникает после слияний компаний.

6. **Золотой молоток**

Использование одной знакомой технологии для решения всех задач. Автор приводит пример с IBM MQ vs Apache Kafka.

7. **Спагетти-код**

Код с множеством goto, копипастой и плохой структурой. Автор дает яркий пример из Linux SCSI driver.

8. **Копипаста**

Эволюция от книг к Google, StackOverflow и теперь ChatGPT. Может быть как экономией времени, так и источником проблем.

9. **Poltergeists (Полтергейсты)**

Это классы, которые появляются ненадолго, делают что-то одно и тут же исчезают. Они обычно:

- Имеют очень короткий жизненный цикл
- Содержат минимум данных и логики
- Существуют только для вызова методов других классов
- Нарушают принципы объектно-oriented дизайна

Пример: класс DatabaseConnector, который только создается, открывает соединение, вызывает один метод другого класса и умирает. 
Вместо этого логичнее было бы интегрировать эту функциональность в основной класс.

В современной разработке это редкость, т.к. современные IDE и code review легко их выявляют

10. **Dead End (Тупик)**

Это ситуация, когда вы используете класс или API, который:

- Не может быть расширен или модифицирован
- Не предоставляет нужной функциональности для будущих потребностей
- Заводит архитектуру в тупик развития

Пример: использование проприетарной библиотеки с закрытым исходным кодом, которая делает 90% того, что нужно, но недостающие 10% критически важны, а библиотека не расширяется.

Обычно выявляется на этапе архитектурного планирования

11. **Input Kludge (Кладж ввода)**

Это костыльная обработка входных данных, когда:

- Система принимает данные в неудобном формате
- Требуется сложная предварительная обработка входных данных
- Логика парсинга размазана по всему коду
- Нет четкого интерфейса для входных данных

Пример: система, которая принимает данные в формате CSV, но парсит их в 15 разных местах кода по-разному, с разными правилами обработки ошибок.

Решается хорошими практиками валидации и парсинга данных

12. **Blind Faith (Слепая вера)**

Это когда разработчики слепо доверяют:

- Внешним API без проверки ответов
- Входным данным без валидации
- Сторонним библиотекам без понимания их работы
- Предположениям о корректности данных

Пример: код, который вызывает внешний API и сразу использует результат, не проверяя статус ответа, наличие данных или их корректность.

Должен автоматически предотвращаться юнит-тестами и интеграционными тестами
...
Подтип — это тип, который можно использовать вместо другого (более общего) типа без ошибок.
Подтип — это "совместимый" тип, который вписывается туда, где ожидается базовый тип.

Разница между подклассом и подтипом:
- Подкласс — это структурное наследование (в ООП, например: class Dog extends Animal).
- Подтип — это поведенческая совместимость: объект ведёт себя так, как ожидается от другого типа.

Все подклассы должны быть подтипами, но не все подтипы обязаны быть подклассами.

Идея в том, что подкласс должен вести себя так, чтобы клиентский код, использующий базовый класс, не **"заметил"** разницы при подстановке экземпляра подкласса.
