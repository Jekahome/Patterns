...
/*
`Template Brige` - отделение интерфейса (группа поведений) от реализации (конкрутное поведение из группы)
...
mod btn{
    use super::User;
    pub struct Button1{// Кнопка на пружине с двумя состояниями
        status: bool,
        spring: bool,
    }
    impl Button1 {
        pub fn new()-> Self{
            Self{status: false, spring: false}
        }
    ...
    pub struct Button2{ // Кнопка на сенсоре с двумя состояниями
        status: bool,
        sensor: String,
    }
    impl Button2{
        fn new(sensor: String)-> Self{
            Self{status: false,sensor}
        }
...
fn main(){
    ...
    let mut client = Client2::new(Box::new(ButtonV2::new(Box::new(StateOnZeroOff::new()),Box::new(Sensor))));
    client.turn_on();
    client.turn_off();
