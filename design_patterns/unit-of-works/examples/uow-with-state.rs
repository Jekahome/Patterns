...
impl NewIdentityEntities{
    pub fn new() -> Self{
        Self{user_collection: vec![], order_collection: vec![]}
    }
    fn register_new_user(&mut self, entity: UserEntityDTO) {
...
pub mod uow{
    use super::domain_entities::{UserEntity, OrderEntity, OrderEntityDTO, UserEntityDTO};
    use super::db_layer;
    use super::{UnitOfWork, NewIdentityEntities };
...
pub mod db_layer{
    ...
    impl IdentityMap{
        ...
        pub fn get_user_by_email(&mut self, email: &str) -> Option<UserEntity>{
            if let Some(user) = self.user_identity.values().into_iter().find(|(user,status)|user.get_email() == email){
                ...
                match *status{
                    Status::DIRTY => {
                        tx.execute(
                            ...
                            params![user.get_name(),user.get_email(),user.get_money(),user.get_id()],
                        ).unwrap();
                    },
                    Status::REMOVED => { 
                        tx.execute(
        ...
        fn load_user_by_email(&mut self, email: &str) -> bool{
            ...
            if let Ok(user) = res{
                self.user_identity.insert(user.get_id(), (user, Status::LOADED));
                return true;
            }
            false
        }
...
use domain_entities::{OrderEntity, UserEntity, UserEntityDTO, OrderEntityDTO};
pub mod domain_entities {
    ...
    impl UserEntityDTO{
        pub fn new(name: &str, email: &str, money: u64) -> Self{
            Self{name: name.to_owned(),email: email.to_owned(), money}
        }
    }
    ...
    impl OrderEntity{
        pub fn new(id: i64, user_id: i64, product: String, total_cost: u64) -> Self{
            Self{id,user_id,product,total_cost}
        }
    }
    ...
    impl OrderEntityDTO{
        pub fn new(user_id: i64, product: &str, total_cost: u64) -> Self{
            Self{user_id, product: product.to_owned(), total_cost}
        }
    }
...
fn main() -> ShortResult<()>{
    ...
    let pool: r2d2::Pool<SqliteConnectionManager> = {
        let manager = SqliteConnectionManager::file("design_patterns/unit-of-works/db/uow_with_state.db");
    ...
    // -- START TRANSACTION --
    for (product,total_cost) in [("TV Toshiba", 77u64),("Bread", 3u64),("Notepad iPad", 19u64)]{
        ...
        if let Some(res) = current_user.get_money().checked_sub(total_cost){
            uow.register_dirty_user(current_user.get_id(), UserEntityDTO::new(current_user.get_name(), current_user.get_email(), res));        
        }else{
            uow.clear();
        ...
        uow.register_new_order(OrderEntityDTO::new(current_user.get_id(), product, total_cost)); 
    }
    uow.commit();
    // -- STOP TRANSACTION --
